...
var config = "./config(asset2).json";   // source file path -- config(QQQ).json,  config(part).json,  config(asset1).json,  config(asset2).json


let width = $('#webglDisplay').width();     // width of modelGroup viewer (left of diplay)
let height = $('#webglDisplay').height();     // height of modelGroup viewer (left of diplay)
...
// sample color list
const colors = [
    '#17111a', '#7a213a', '#e14141', '#FF5E13','#ffa070', '#c44d29', 
	'#ffbf36', '#753939', '#cf7957',  '#39855a', 
	'#83e04c', '#dcff70', '#243b61', '#3898ff', '#6eeeff', 
	'#bf3fb3', '#ff80aa', '#7884ab', '#b2bcc2', '#ffffff'
];
...
// hover and click event of sample colors
function mOver(obj) {
...
// scenes and renderers for viewing each model
var scene_item = [];
var renderer_item = [];

var initScale = [0.5, 0.5, 0.5];
...
function modelpieceDisplay() {
    ...
    $.getJSON(config, function (data) {
        ...
        menuData.forEach(function (item, index) {
            ...
            scene_item[index] = new THREE.Scene();
            renderer_item[index] = new THREE.WebGLRenderer({ alpha: true });
            renderer_item[index].setClearColor("#000000", 0);
            ...
            scene_item[index].add(pointlight_1_item);
            const pointlight_2_item = new THREE.PointLight("#fff", 1, 2000);
            pointlight_2_item.position.set(-1000, 1000, 1000);
            scene_item[index].add(pointlight_2_item);
            const pointlight_3_item = new THREE.PointLight("#fff", 1, 3000);
            pointlight_3_item.position.set(0, 0, -2000);
            scene_item[index].add(pointlight_3_item);
            const pointlight_4_item = new THREE.PointLight("#fff", 1, 2000);
            pointlight_4_item.position.set(-2000, 0, 0);
            scene_item[index].add(pointlight_4_item);
            const pointlight_5_item = new THREE.PointLight("#fff", 1, 2000);
            pointlight_5_item.position.set(0, -2000, 0);
            scene_item[index].add(pointlight_5_item);

            const controls_item = new THREE.OrbitControls(camera_item, renderer_item[index].domElement);
            controls_item.domElement = false;
            ...
            function modelLoader_item(scene, name, src, color) {
                ...
                loader.load(src, function (geometry) {
                    ...
                    if(index == 0) {
                        ...
                        mesh.scale.set(initScale[0], initScale[0], initScale[0]);
                    } else {
                        //console.log("#temp_" + xyz[maxIndex(bbox.max)], index, $("#temp_" + xyz[maxIndex(bbox.max)]).val());
            ...
            function render() {
                requestAnimationFrame(render);
                renderer_item[index].render(scene_item[index], camera_item);
            }
            modelLoader_item(scene_item[index], item.value, item.src, item.color);
        });
    });
}
...
function onclick_col_sample(index) {
    ...
    if(setId == 1 || setId == 3) { // only 2nd part and 4th part are editable.
...
// scene and other values for previewing models
var scene_preview = new THREE.Scene();
const renderer_preview = new THREE.WebGLRenderer({ alpha: true });
renderer_preview.setClearColor("#000000", 0);

const pointlight_1_preview = new THREE.PointLight("#eee", 1, 3000);
pointlight_1_preview.position.set(0, 0, 2000);
const pointlight_2_preview = new THREE.PointLight("#eee", 1, 2000);
...
const pointlight_4_preview = new THREE.PointLight("#eee", 1, 2000);
pointlight_4_preview.position.set(-2000, 0, 0);
const pointlight_5_preview = new THREE.PointLight("#eee", 1, 2000);
    ...
    if(setId != -1) {
        $.getJSON(config, function (data) {
            ...
            function modelLoader_preview(scene, name, src, color) {
                ...
                loader.load(src, function (geometry) {
                    ...
                    if(setId == 0) {
                        ...
                        $("#temp_y").val(bbox.max.y);
                        $("#temp_z").val(bbox.max.z);
                        mesh.scale.set(2 * initScale[0], 2 * initScale[0], 2 * initScale[0]);
                    } else {
            ...
            function render_preview() {
                requestAnimationFrame(render_preview);
            ...
            if(color == "") {
                ...
                console.log(1)
                modelLoader_preview(scene_preview, menuData[setId].value, menuData[setId].src, color);
            }
        });
        ...
        if(color != "") { 
            $.getJSON(config, function (data) {
                ...
                renderer_item[setId].setClearColor("#000000", 0);
                ...
                const controls = new THREE.OrbitControls(camera, renderer_item[setId].domElement);
                ...
                function render() {
                    requestAnimationFrame(render);
                    renderer_item[setId].render(scene_item[setId], camera);
                }
                modelLoader_item(scene_item[setId], menuData[setId].value, menuData[setId].src, color);
            });
        }
    }
}
